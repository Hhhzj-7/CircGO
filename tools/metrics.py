...
def fmax_pytorch(y_pred, y_true, device):
    with torch.no_grad():
        ...
        precision_, recall_ = torch.tensor(0.0).to(device), torch.tensor(0.0).to(device)
        for threshold in (c / 100 for c in range(101)):
            ...
            with warnings.catch_warnings():
                ...
                avg_recall = torch.mean(recall[torch.bitwise_not(torch.isnan(recall))])
            ...
            try:
                ...
                if fmax_t > fmax_:
                    fmax_ = fmax_t
                    precision_ = avg_precision
                    recall_ = avg_recall
                    threshold_ = threshold
            except ZeroDivisionError:
                pass
        return fmax_.cpu().numpy(), precision_.cpu().numpy(), recall_.cpu().numpy(), threshold_
